...
def cxOrdered_by_class(ind1, ind2,period_number_list):
    """Executes an ordered crossover (OX) on the input
    individuals. The two individuals are modified in place. This crossover
    expects :term:`sequence` individuals of indices, the result for any other
    type of individuals is unpredictable.
    ...
    storage1 = ind1
    storage2 = ind2
    for number,periods in enumerate(period_number_list):
        '''
        if number==0:
            ind1=storage1[:periods]
            ind2=storage2[:periods]
        elif number==len(period_number_list)-1:
            ind1 = storage1[-periods:]
            ind2 = storage2[-periods:]
        else:
        '''
        ind1 = storage1[sum(period_number_list[:number]):sum(period_number_list[:number])+periods]
        ind2 = storage2[sum(period_number_list[:number]):sum(period_number_list[:number])+periods]
        ...
        holes1, holes2 = [True] * size, [True] * size
        Beginning_number=min(ind1)
        for i in range(size):
            if i < a or i > b:
                holes1[ind2[i]-Beginning_number] = False
                holes2[ind1[i]-Beginning_number] = False

        # We must keep the original values somewhere before scrambling everything
        temp1, temp2 = ind1, ind2
        k1, k2 = b + 1, b + 1
        for i in range(size):
            if not holes1[temp1[(i + b + 1) % size]-Beginning_number]:
                ind1[k1 % size] = temp1[(i + b + 1) % size]
                k1 += 1

            if not holes2[temp2[(i + b + 1) % size]-Beginning_number]:
                ind2[k2 % size] = temp2[(i + b + 1) % size]
                k2 += 1
        ...
        storage1[sum(period_number_list[:number]):sum(period_number_list[:number]) + periods]=ind1
        storage2[sum(period_number_list[:number]):sum(period_number_list[:number]) + periods]=ind2
...
__all__ = ['cxOnePoint', 'cxTwoPoint', 'cxUniform', 'cxPartialyMatched',
           ...
           'cxESBlend', 'cxESTwoPoint',"cxOrdered_by_class"]
