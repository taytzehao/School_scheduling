...
def hamming_distance(ind1,ind2):
    return len([True for a,b in zip(ind1,ind2) if a!=b])
...
def eaSimple(population, toolbox, cxpb, mutpb, ngen,fitness_stop, stats=None,
    ...
    for gen in range(1, ngen + 1):
        ...
        fitnesses = toolbox.map(toolbox.evaluate, invalid_ind)
        fitnesses2 = list(toolbox.clone(fitnesses))
        ...
        if verbose:
            print (logbook.stream)
        if fitness_stop in fitnesses2:
            return population, logbook

    return population, logbook

def inject_new_individual(individual_function,population,injection_ratio):
    size_range=range(len(population))
    for size in range(int(len(population)*injection_ratio)):
        population[random.choice(size_range)]=individual_function()
    return population
...
def eaSimple2(populations, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__):
    ## This version of the easimple allows for migration to occur
    logbook = tools.Logbook()
    logbook.header = ['gen', 'nevals'] + (stats.fields if stats else [])
